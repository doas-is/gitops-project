...
Each stage emits events to the UI via add_event() and writes to the AuditLogger.
"""
...
# Production would use MicroVMOrchestrator from azure_setup.py
# ─────────────────────────────────────────────────────────────────
...
class SecureAnalysisPipeline:
    """
    Coordinates all 8 stages. Each stage:
      - Creates its microVM record
      - Does its work
      - Destroys its microVM when done
    After stage 8, all remaining VMs are purged. Audit log survives.
    """
    ...
    def _decrypt_and_parse(self, payload: EncryptedFilePayload) -> Optional[ASTPayload]:
        ...
        try:
            ...
            with self.encryptor.decrypt_context(enc) as plaintext_bytes:
                ...
                # Use multi-language dispatcher for all extensions
                result = parse_source_bytes(
                    plaintext_bytes, ext,
                    payload.file_index, payload.header.task_id,
                )
                if result is None:
                    # Unsupported extension — create minimal payload
            ...
            logger.error("Parse error file %d: %s", payload.file_index, type(e).__name__)
            return None
    ...
    async def run_analysis(self, repo_url: str, task_id: Optional[str] = None) -> dict:
        ...
        fetcher = SecureFetcherAgent(mtls_config=mtls_cfg, parser_host="localhost",
                                     parser_port=8443, use_local_kms=True)
        ...
        try:
            ...
            return self._error(task_id, f"Fetch failed: {type(e).__name__}", start)
        ...
        _emit("stage_update", {"task_id": task_id, "stage": "fetch", "status": "complete",
                                ...
                                "message": f"Fetched {len(encrypted_payloads)} files (AES-256-GCM encrypted)"})

        if not encrypted_payloads:
            vms.destroy_all("task_failed")
            return self._error(task_id, "No files fetched", start)

        # ── Stage 2: PARSE ────────────────────────────────────────
        vm_parse = vms.create("ast_parser")
        _emit("stage_update", {"task_id": task_id, "stage": "parse",
                                "status": "running", "message": "Parsing ASTs (semantic stripping)..."})

        ast_payloads: List[ASTPayload] = []
        for enc in encrypted_payloads:
            r = self._decrypt_and_parse(enc)
            if r:
                ast_payloads.append(r)
        ...
        self._audit.task_stage(task_id, "parse", "complete",
                               f"{len(ast_payloads)} ASTs — all strings/names stripped")
        _emit("stage_update", {"task_id": task_id, "stage": "parse", "status": "complete",
                                "ast_count": len(ast_payloads),
                                "message": f"Parsed {len(ast_payloads)} files — identifiers hashed, strings removed"})

        # ── Stage 3: IR BUILD ─────────────────────────────────────
        vm_ir = vms.create("ir_builder")
        _emit("stage_update", {"task_id": task_id, "stage": "ir",
                                "status": "running", "message": "Building language-agnostic IR..."})
        ...
        _emit("stage_update", {"task_id": task_id, "stage": "ir", "status": "complete",
        ...
        }, severity="error" if policy_decision.decision == "REJECT" else
               "warning" if policy_decision.hitl_required else "info")

        # ── Stage 6: STRATEGY ─────────────────────────────────────
        vm_strat = vms.create("strategy_agent")
        ...
        vms.destroy(vm_strat, "stage_complete")
        self._audit.deployment_strategy(task_id, strategy.method,
                                        "; ".join(strategy.reasoning))
        _emit("stage_update", {
            "task_id": task_id, "stage": "strategy", "status": "complete",
            "method": strategy.method,
            "primary_tool": strategy.primary_tool,
            "secondary_tool": strategy.secondary_tool,
            "reasoning": strategy.reasoning[:2],
            "message": f"Strategy: {strategy.method.upper()} — "
        ...
        _emit("stage_update", {"task_id": task_id, "stage": "deploy",
                                "status": "running",
                                "message": "Deploying resources to Azure (ephemeral environment)..."})

        deploy_result = await self.deploy_agent.deploy(iac_bundle)

        vms.destroy(vm_deploy, "task_complete")

        # ── TEARDOWN: destroy all remaining VMs ───────────────────
    ...
    def _error(task_id: str, msg: str, start: float) -> dict:
        _emit("task_failed", {"task_id": task_id, "error": msg,
                               "stage": "failed", "message": msg}, severity="error")
        return {
